...
from .event_service import EventService, _split_timestamp
...
class ZoneService:
    ...
    def _sync_setpoint_to_hardware(self, zone_name: str, setpoint: float) -> None:
        """Sync setpoint to hardware controller for simulation purposes."""
    ...
    def update_zone(self, zone_name: str, payload: ZoneUpdateRequest) -> ZoneStatusModel:
        ...
        if payload.target_setpoint_f is not None:
            ...
            if current and control_mode == "AUTO":
                ...
                # Get current scheduled setpoint to store for boundary detection
                ...
                repositories.update_zone_status(
                    ...
                    setpoint_override_scheduled_value=scheduled_setpoint,
                )
                self._sync_setpoint_to_hardware(zone_name, payload.target_setpoint_f)
            else:
    ...
    def update_zone_schedule(
        ...
        logger.info(f"📅 Updating schedule for {zone_name}")
        normalized = self._normalize_request_entries(payload.entries)
        repositories.replace_zone_schedule(zone_name, normalized)
        # Force clear boundary overrides when manually updating schedule
        self._refresh_auto_setpoints([zone_name], clear_boundary_overrides=True)
    ...
    def _sync_auto_setpoint(self, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if override_at is not None and override_mode is not None:
            ...
            if override_mode == "timed" and override_until is not None:
                try:
                    ...
                    if datetime.utcnow() >= until_time:
                        ...
                        row["SetpointOverrideAt"] = None
                        row["SetpointOverrideMode"] = None
                        row["SetpointOverrideUntil"] = None
                        row["SetpointOverrideScheduledValue"] = None
                        return row
                except Exception as e:
            ...
            if override_mode == "boundary":
                # Check if the scheduled setpoint has changed since the override was set
                override_scheduled_value = row.get("SetpointOverrideScheduledValue")
                if override_scheduled_value is not None:
                    # Compare current schedule to the schedule value at time of override
                    if abs(scheduled_setpoint - override_scheduled_value) > 0.05:
                        # Schedule has changed - this is a schedule boundary, clear override
                        logger.info(f"Schedule changed for {zone_name}: {override_scheduled_value}°F → {scheduled_setpoint}°F, clearing boundary override")
                        repositories.update_zone_status(zone_name,
                                                       target_setpoint_f=scheduled_setpoint,
                                                       clear_override=True)
                        current_setpoint = scheduled_setpoint
                    ...
                    # Fallback: no scheduled value stored (legacy), clear on setpoint mismatch
        ...
        if current_setpoint is None or abs(current_setpoint - scheduled_setpoint) > 0.05:
            ...
            self._sync_setpoint_to_hardware(zone_name, scheduled_setpoint)
            current_setpoint = scheduled_setpoint
    ...
    def _refresh_auto_setpoints(self, zone_names: Optional[Iterable[str]] = None, clear_boundary_overrides: bool = False) -> None:
        """
        Apply scheduled setpoints to AUTO zones after schedule updates.

        Args:
            zone_names: Specific zones to refresh, or None for all zones
            clear_boundary_overrides: If True, force clear all boundary overrides regardless of value comparison
        """
        ...
        for zone in targets:
            ...
            if not row or row.get("ControlMode") != "AUTO":
                logger.info(f"Skipping {zone}: not in AUTO mode (mode={row.get('ControlMode') if row else 'no row'})")
                continue

            # If manually updating schedule, force clear boundary overrides
            if clear_boundary_overrides and row.get("SetpointOverrideMode") == "boundary":
                scheduled_setpoint = self._resolve_scheduled_setpoint(zone)
                if scheduled_setpoint is not None:
                    logger.info(f"Manually cleared boundary override for {zone}, applying schedule {scheduled_setpoint}°F")
                    repositories.update_zone_status(zone,
                                                   target_setpoint_f=scheduled_setpoint,
                                                   clear_override=True)
            ...
            normalized = _normalize_row_keys(row)
            old_setpoint = normalized.get("TargetSetpoint_F")
            updated_row = self._sync_auto_setpoint(normalized)
            new_setpoint = updated_row.get("TargetSetpoint_F")

            # If the row was modified, it means the setpoint was updated
            if old_setpoint != new_setpoint:
                logger.info(f"Setpoint changed for {zone}: {old_setpoint}°F → {new_setpoint}°F")
                updated_zones.append(zone)
            else:
                logger.info(f"No setpoint change for {zone} (still {old_setpoint}°F)")

        # Log if any zones were updated
        if updated_zones:
            logger.info(f"✓ Schedule update applied to AUTO zones: {', '.join(updated_zones)}")
        else:
            logger.info(f"No AUTO zones required setpoint updates")
    ...
    def get_zones_history_batch(
        ...
        # Optimized batch query - fetch all zones in one SQL query
        start_time = time.perf_counter()
        since_naive: datetime
        until_naive: Optional[datetime] = None
        normalized_span = max(1, min(span_days, 31))
        ...
        # Fetch all events for all zones in one query using IN clause
        from ..database import get_connection

        placeholders = ','.join('?' * len(sorted_key))
        query = f"""
            SELECT *
            FROM EventLog
            WHERE Source IN ({placeholders})
              AND Timestamp >= ?
        """
        params = list(sorted_key) + [since_naive.isoformat()]

        if until_naive:
            query += " AND Timestamp <= ?\n"
            params.append(until_naive.isoformat())

        query += f" ORDER BY Source, Timestamp DESC\n LIMIT ?"
        params.append(limit * len(sorted_key))  # Get limit per zone
        ...
        events_by_zone: Dict[str, List[Dict[str, Any]]] = {zone: [] for zone in sorted_key}
        for row in all_rows:
            zone_name = row.get("Source")
            if zone_name in events_by_zone:
                events_by_zone[zone_name].append(row)

        # Process each zone's history
        histories: Dict[str, List[EventLogModel]] = {}
        for zone in sorted_key:
            zone_events = events_by_zone.get(zone, [])

            # Convert to EventLogModel and add room names
            history = []
            for row in zone_events:
                row["RoomName"] = settings.zone_room_map.get(zone, zone)
                date_part, time_part = _split_timestamp(row.get("Timestamp"))
                row["EventDate"] = date_part
                row["EventTime"] = time_part
                history.append(EventLogModel.model_validate(row))

            # Add current status as latest sample
            status_row = repositories.get_zone_status(zone)
            if status_row:
                decorated = self._decorate_row(status_row)
                latest_timestamp = decorated.get("UpdatedAt")
                if not latest_timestamp:
            ...
            # Sort and downsample
            history.sort(key=lambda item: item.timestamp)
            histories[zone] = self._downsample_history(history, max_samples)

        duration = time.perf_counter() - start_time
        logger.info(
            "history.batch zones=%s window=%s rows=%s duration=%.3fs",
            len(sorted_key),
            f"day={day},span={normalized_span}" if day else f"hours={hours}",
