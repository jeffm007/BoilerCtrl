...
def sync_events(
    ...
    since_timestamp: Optional[str] = Query(None, description="Get events after this timestamp"),
    limit: int = Query(1000, ge=1, le=10000)
):
    """
    Sync endpoint for NAS to fetch incremental event_log updates.
    Returns events in chronological order (oldest first).
    """
    from backend.repositories import get_connection
    ...
    try:
        with get_connection() as conn:
            if since_id is not None:
                ...
                query = """
                    SELECT Id, Timestamp, Source, Event, ZoneRoomTemp_F, PipeTemp_F, OutsideTemp_F, DurationSeconds
                    FROM EventLog
                    WHERE Id > ?
                    ORDER BY Id ASC
                    LIMIT ?
                ...
                query = """
                    ...
                    LIMIT ?
                """
                params = (seven_days_ago, limit)

            cursor = conn.execute(query, params)
            events = cursor.fetchall()  # Already returns list of dicts due to row_factory

            return {"events": events, "count": len(events)}

    except sqlite3.Error as e:
        logger.error(f"Database error in sync_events: {e}")
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    except Exception as e:
        logger.error(f"Error in sync_events: {e}")
