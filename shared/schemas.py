"""
Shared schemas for sync protocol between Pi controller and NAS web dashboard.
"""
...
class SyncEventType(str):
    """Event types for sync protocol."""
...
class SyncMessage(BaseModel):
    """Base message for sync protocol between Pi and NAS."""
    event_type: str
    timestamp: str  # ISO format
    sequence_id: int  # Monotonic sequence number for ordering
    payload: Dict


class ZoneStateUpdatePayload(BaseModel):
    """Payload for zone state changes from Pi to NAS."""
    zones: List[ZoneStatusModel]
    system: Optional[SystemStatusModel] = None


class CommandRequestPayload(BaseModel):
    """Command from NAS to Pi."""
    command_id: str  # UUID for tracking
    zone_name: Optional[str] = None  # None for system-wide commands
    command_type: Literal["zone_command", "zone_update", "uniform_setpoint"]
    command_data: Dict  # Contains ZoneCommandRequest, ZoneUpdateRequest, etc.


class CommandResponsePayload(BaseModel):
    """Response from Pi after executing command."""
    command_id: str
    success: bool
...
class FullSyncRequestPayload(BaseModel):
    """Request full state sync (on reconnect or initial load)."""
    last_sequence_id: Optional[int] = None  # Resume from sequence if available


class FullSyncResponsePayload(BaseModel):
    """Full state dump from Pi."""
    zones: List[ZoneStatusModel]
    system: SystemStatusModel
    recent_events: List[EventLogModel]
